...
import { ChevronRight, HelpCircle, ListChecks, Settings, ShoppingBag } from '@lucide/vue'
...
function openTasks()   { closeAll(); toggleLogs(true, 'logs'); tasksOpen.value = true }

const anyOpen = computed(
  () => logsOpen.value || shopOpen.value || settingsOpen.value || helpOpen.value || tasksOpen.value,
)
...
<template>
  ...
  <div class="absolute left-[320px] top-[56px] z-10 flex flex-col drawer-transition" :style="bookmarkStyle">
    <button
      ...
      class="w-7 py-4 bg-zinc-900 hover:bg-black text-white rounded-tr-xl border-t border-r border-white/10 flex flex-col items-center gap-1.5 group"
      data-no-drag
    >
    ...
    <button
      ...
      title="Task progress"
      class="w-7 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-br-xl border-b border-r border-white/10 flex items-center justify-center group"
      data-no-drag
    >
      <ListChecks class="w-3.5 h-3.5 text-zinc-500 group-hover:text-emerald-400 transition-colors" />
    </button>
  </div>
  ...
  <div class="absolute left-[320px] bottom-[130px] z-10 flex flex-col drawer-transition" :style="bookmarkStyle">
    <button
      ...
      class="w-7 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-tr-xl border-t border-r border-white/10 flex items-center justify-center group"
      data-no-drag
    >
      <HelpCircle class="w-3.5 h-3.5 text-zinc-500 group-hover:text-white transition-colors" />
    </button>
    <button
      @click="openSettings"
      :aria-label="$t('bookmark.settings')"
      class="w-7 py-3 bg-zinc-800 hover:bg-zinc-700 text-white border-r border-white/10 flex items-center justify-center group"
      data-no-drag
    >
    ...
    <button
      ...
      class="w-7 py-3 bg-zinc-900 hover:bg-black text-white rounded-br-xl border-b border-r border-white/10 flex items-center justify-center group"
      data-no-drag
    >
